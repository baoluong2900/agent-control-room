...
function FloatingTask({ card }: { card: OperationCardView }) {
  const Icon = card.icon;
  const style = { "--task-accent": card.accent } as CSSProperties;

  return (
    <article className={`floating-task task-${card.id}`} style={style}>
      <header>
        <span>
          <Icon size={12} />
          {card.title}
    ...
    >
      ...
      <div className="workspace-canvas-host">
        <Canvas
          ...
          shadows
        >
          <fog attach="fog" args={["#050913", 28, 52]} />
          <CameraFit />
          <ambientLight intensity={0.2} />
          <hemisphereLight args={["#78B8FF", "#03060E", 0.4]} />
          <directionalLight
            castShadow
            intensity={0.9}
            position={[9, 15, 7]}
          ...
          />
          <directionalLight color="#38E7D0" intensity={0.28} position={[-9, 9, -7]} />
          <pointLight color="#36A8FF" distance={16} intensity={0.52} position={[0, 5.2, 0]} />
          <pointLight color="#FFD45A" distance={11} intensity={0.18} position={[0, 1.8, 0]} />
      ...
      {cards.map((card) => (
        <FloatingTask card={card} key={card.id} />
      ))}
...
function buildOperationCards(history: AgentRunRecord[], activeStatus: AgentStatus): OperationCardView[] {
  return operationCards.map((card) => {
    ...
    const accent = zoneById.get(card.id)?.accent ?? "#4AA3FF";
