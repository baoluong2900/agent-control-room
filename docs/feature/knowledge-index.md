...
- [ ] Scan chính repo này → `src/contracts` là local node, không phải external package.
- [ ] Scan repo lớn → thấy progress tăng, và cancel được giữa scan.
- [ ] Scan lần hai ngay sau lần đầu → nhanh hơn rõ rệt, và log/số liệu cho thấy hầu hết file được bỏ qua.
- [ ] Import viết trong comment không xuất hiện thành edge trong graph.
- [ ] Search "workflow" → file tên `workflow-*.ts` xếp trên file chỉ nhắc chữ đó trong mô tả.
- [ ] `npm test` xanh.
