...
export function WorkflowsModule() {
  ...
  const [awaitingApprovalByWorkflow, setAwaitingApprovalByWorkflow] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    const unsubscribe = window.agentic.events.subscribeWorkflow((event) => {
      ...
      if (event.type === "workflow:run-started" && event.workflowRunId) {
        ...
        setAwaitingApprovalByWorkflow((prev) => {
          const next = { ...prev };
      ...
      if (event.type === "workflow:run-finished") {
        // A run parked on an approval gate reports "finished" so the UI stops
        // showing it as busy, but its run id must stay so Approve can resume it.
        const parked = event.status === "waiting-approval";
        setRunningId((current) => (current === event.workflowId ? null : current));
        if (parked) {
          setAwaitingApprovalByWorkflow((prev) => ({ ...prev, [event.workflowId]: event.workflowRunId }));
        } else {
  ...
  /**
   * Every IPC call below can reject (validation, database, cancelled dialog).
   * Without a catch the rejection is unhandled and the UI silently does nothing,
   * so each one reports through the same `error` banner as run/export.
   */
  ...
  /** Signs off the open gate; the main process resumes the remaining steps. */
  async function handleApprove(workflowId: string) {
    const runId = awaitingApprovalByWorkflow[workflowId];
    if (!runId) return;
    setRunningId(workflowId);
    try {
      await window.agentic.workflows.approve(runId);
    } catch (approveError) {
  ...
  async function handleReject(workflowId: string) {
    const runId = awaitingApprovalByWorkflow[workflowId];
    if (!runId) return;
    const reason = window.prompt("Reason for rejecting this step? (optional)") ?? undefined;
    try {
      await window.agentic.workflows.reject(runId, reason);
    } catch (rejectError) {
  ...
  /**
   * The list copy can be stale if a run finished (or another window saved) since
   * the last refresh, so read the definition back before editing it.
   */
  async function handleEdit(workflow: WorkflowDefinition) {
    try {
      const fresh = await window.agentic.workflows.get(workflow.id);
      setEditor({ open: true, workflow: fresh ?? workflow });
    } catch (editError) {
      setError(editError instanceof Error ? editError.message : String(editError));
      setEditor({ open: true, workflow });
    }
  }
  ...
  /**
   * The scheduler already ticks on its own timer in the main process; this just
   * forces a check now so a "Daily, 9:00 AM" workflow can be verified without
  ...
  async function handleRunDueSchedules() {
    ...
    setNotice(null);
    try {
      const fired = await window.agentic.workflows.runDueSchedules();
      const names = fired
        .map((id) => workflows.find((workflow) => workflow.id === id)?.name ?? id)
        .join(", ");
      setNotice(fired.length === 0 ? "No scheduled workflow is due right now." : `Started: ${names}`);
      await refreshAll();
    } catch (scheduleError) {
    ...
    <div className="wf-page">
      <section className="wf-hero">
        ...
        <div className="wf-hero-actions">
          ...
          <button
            className="wf-ghost-btn"
            disabled={checkingSchedules}
            onClick={() => void handleRunDueSchedules()}
            title="Run any scheduled workflow that is due now"
          >
            <AlarmClock size={14} /> {checkingSchedules ? "Checking…" : "Check Schedules"}
          </button>
      ...
      {notice && (
        <div className="wf-banner-notice">
          <span>{notice}</span>
      ...
      <div className={`wf-layout ${selected ? "with-detail" : ""}`}>
        ...
        {selected && (
          <WorkflowDetailPanel
            ...
            awaitingApproval={Boolean(awaitingApprovalByWorkflow[selected.id])}
            onClose={() => setSelectedId(null)}
            onEdit={() => void handleEdit(selected)}
