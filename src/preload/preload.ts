...
const api: AgenticDesktopApi = {
  ...
  settings: {
    ...
    openProviderSite: (input: ProviderConnectionAuthRequest) => ipcRenderer.invoke("settings:open-provider-site", input),
  },
  agents: {
  ...
  workflows: {
    ...
    webhookStatus: () => ipcRenderer.invoke("workflow:webhook-status"),
    rotateWebhookToken: () => ipcRenderer.invoke("workflow:rotate-webhook-token"),
